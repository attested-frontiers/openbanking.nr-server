...
app.get('/health', async (req, res) => {
  try {
    ...
    if (!isDbHealthy) {
      return res
        .status(500)
        .json({ status: 'error', message: 'Database is down' });
    }
...
app.get('/commitments/purge', async (_, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: 'Failed to purge commitments', details: error.message });
  }
});
...
app.get('/process-auth', async (req, res) => {
  ...
  try {
    ...
    const paymentResponse = await executeDomesticPayment(
      paymentData,
      consentId,
...
app.get('/token-status', (req, res) => {
  res.json({
    ...
    accessTokenPreview: currentToken
      ? `${currentToken.access_token.substring(0, 20)}...`
      : null,
  });
});
...
app.post('/noir-inputs', async (req, res) => {
  try {
    ...
    if (!signature || !rawPayload) {
      return res
        .status(400)
        .json({ error: 'Signature and dataToVerify are required' });
    }
